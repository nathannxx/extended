...
RVPATH=/data/adb/rvnx/${MODPATH##*/}.apk
		...
		if ! op=$(pmex install-commit "$SES"); then
			...
			if echo "$op" | grep -q -e INSTALL_FAILED_VERSION_DOWNGRADE -e INSTALL_FAILED_UPDATE_INCOMPATIBLE; then
				...
				if [ "$IS_SYS" = true ]; then
					...
					mkdir -p /data/adb/rvnx/empty /data/adb/post-fs-data.d
					echo "mount -o bind /data/adb/rvnx/empty $BASEPATH" >"$SCNM"
					chmod +x "$SCNM"
...
ui_print "* Done"
ui_print "  by nathan (github.com/nathannxx)"
ui_print " "
